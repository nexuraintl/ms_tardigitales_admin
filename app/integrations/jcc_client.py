...
class JccClient:
    ...
    async def consultar_registro(self, documento: str, tipo_tarjeta: str = "contadores", tipo: str = "") -> Dict[str, Any]:
        ...
        if not documento_limpio:
            return {"disponibles": [], "pdf": None, "encontrado": False, "error": "Número de documento o NIT no especificado."}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "error" in data and data["error"]:
                    return {"encontrado": False, "data": None, "error": data["error"]}
                
                disponibles = data.get("disponibles", [])
                pdf = data.get("pdf")
                
                if not disponibles and not pdf:
                    return {"encontrado": False, "data": None}

                raw_item = disponibles[0] if disponibles else {}
                
                # Normalización de la fotografía Base64 de la JCC (la API llama 'pdf' a la imagen JPEG)
